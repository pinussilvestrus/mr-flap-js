...
Canvas.prototype.collisionDetection = function () {
  ...
  for (let i = 0; i < oObst.length; i++) {
    ...
    // TODO: rethink x value with proximity
    if (oBird.x === oObst[i].shape.x)
      if (oObst[i].shape.y === 0)
        if (oBird.y - oBird.height < obstHeight)
          console.log('hit up'); // hit the top obstacle
        else if (oBird.y + oBird.height > this.canvas.height - obstHeight)
          console.log('hit low'); // hit the bottom obstacle
  }
};
